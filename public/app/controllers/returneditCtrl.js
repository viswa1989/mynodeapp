/* global _ */
/* global angular */
angular.module("returneditCtrl", []).controller("ReturneditController", ($scope, $rootScope, $routeParams, $sce, $uibModal, $log, AuthService,
  ...
  $scope.getDeliverybyid = function (del) {
    DeliveryreturnService.getDeliveryeditview(del, (result) => {
      if (result !== null && angular.isDefined(result.success)) {
        if (result.success) {
          if (angular.isDefined(result.data) && result.data !== null && angular.isDefined(result.data.Order) && angular.isDefined(result.data.Delivery)
            ...
            angular.forEach(selectedOrder.return_delivery, (deldata, dind) => {
              if (angular.isDefined(deldata._id) && deldata._id == result.data.Delivery._id) {
                selectedOrder.return_delivery.splice(dind, 1);
                $scope.deliveryreturnForm = angular.copy(result.data.Delivery);

                $scope.deliveryreturnData.delivery_company_name = angular.copy(result.data.Delivery.delivery_company_name);
                $scope.deliveryreturnData.delivery_address_line = angular.copy(result.data.Delivery.delivery_address_line);
                $scope.deliveryreturnData.delivery_city = angular.copy(result.data.Delivery.delivery_city);
                $scope.deliveryreturnData.delivery_pincode = angular.copy(result.data.Delivery.delivery_pincode);
  ...
  $scope.initializeDeliverydetails = function () {
    ...
    $scope.deliveryreturnData.showButton = false;
    $scope.deliveryreturnForm = {};
    $scope.deliveryreturnData.updateconfirm = false;
  ...
  $scope.printThisdelivery = function () {
    ...
    retData.outwardData = $scope.deliveryreturnForm.outward_data;
  ...
  $scope.saveDelivery = function (opt) {
    validateField.validate($scope.deliveryreturnForm, orderfield, ordermsgData).then((orderMsg) => {
      if (angular.isDefined(orderMsg) && orderMsg !== null && orderMsg !== "") {
        ...
        validateField.validateGroup($scope.deliveryreturnForm.outward_data, outwardfield, outwardmsgData).then((outwardMsg) => {
          if (angular.isDefined(outwardMsg) && outwardMsg !== null && outwardMsg !== "") {
            Notification.error(outwardMsg);
          } else {
            validateOutwarddata($scope.deliveryreturnForm.outward_data).then((valid) => {
              if (angular.isDefined(valid) && valid) {
                $rootScope.orderpageLoader = true;
                const obj = {};
                obj.deliveryreturnForm = angular.copy($scope.deliveryreturnForm);

                DeliveryreturnService.update(obj, (result) => {
                  if (result !== null && angular.isDefined(result) && angular.isDefined(result.success)) {
                    if (result.success) {
                      if (angular.isDefined(result.data) && angular.isDefined(result.data._id)) {
                        ...
                        $scope.deliveryreturnData.updateconfirm = true;
                        if (opt === "print") {
                          $timeout(() => {
  ...
  function getCurrentdelivery () {
    ...
    angular.forEach($scope.deliveryreturnForm.inwards, (inward, inx) => {
      angular.forEach($scope.deliveryreturnForm.outward_data, (outdata, odx) => {
        if (angular.isDefined(outdata.inward_id) && angular.isDefined(outdata.inward_data_id) && outdata.inward_id === inward.inward_id &&
        ...
        if (odx === $scope.deliveryreturnForm.outward_data.length - 1 && inx === $scope.deliveryreturnForm.inwards.length - 1) {
          deferred.resolve($scope.deliveryreturnForm.outward_data);
        }
      });
    });
