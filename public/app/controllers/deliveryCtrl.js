/* global _ */
/* global angular */
angular.module("deliveryCtrl", []).controller("DeliveryController", ($scope, $rootScope, $routeParams, $sce, $uibModal, $log,
  ...
  $scope.deliveryData = {};
  $scope.deliveryForm = {};
  $scope.deliveryData.customers = [];
  $scope.deliveryData.orders = [];
  ...
  $scope.initializeDeliverydetails = function () {
    ...
    $scope.deliveryData.detailformSumission = false;
    $scope.deliveryData.showButton = false;
    $scope.deliveryData.customerSearch = false;
    $scope.deliveryData.customer_mobile = "";
    $scope.deliveryForm = {};
  ...
  $scope.getcustomerDetails = function () {
    $scope.deliveryData.customers = [];
    $scope.deliveryData.customerSearch = true;
    if (angular.isDefined($scope.deliveryData.customer_mobile) && $scope.deliveryData.customer_mobile !== ""
      && $scope.deliveryData.customer_mobile.length > 4) {
      const obj = {};
      obj.customer_mobile = angular.copy($scope.deliveryData.customer_mobile);
      ...
      CustomerService.getcustomerbyDivision(obj, (result) => {
        if (result !== null && angular.isDefined(result.success) && result.success && angular.isDefined(result.data) && 
          ...
          $scope.deliveryData.customers = angular.copy(result.data.Customer);
        }
      }, (error) => {
        if (error !== null && angular.isDefined(error.message)) {
          Notification.error(error.message);
  ...
  $scope.selectCustomer = function (customer) {
    ...
    if (angular.isDefined(customer) && angular.isDefined(customer._id) && angular.isDefined(customer.name) && customer._id !== "") {
      ...
      $scope.deliveryForm.customer_id = angular.copy(customer._id);
      $scope.deliveryForm.customer_name = angular.copy(customer.name);
      $scope.deliveryForm.customer_mobile_no = angular.copy(customer.mobile_no);
      $scope.deliveryData.customer_mobile = angular.copy(customer.mobile_no);
      $scope.deliveryForm.gstin = angular.copy(customer.gstin);
      if (angular.isDefined(customer.customer_alternate_no)) {
        $scope.deliveryForm.customer_alternate_no = angular.copy(customer.customer_alternate_no);
      }
      if (angular.isDefined(customer.is_favourite)) {
        $scope.deliveryForm.is_favourite = angular.copy(customer.is_favourite);
      }

      $scope.deliveryForm.delivery_company_name = angular.copy(customer.name);

      angular.forEach(customer.address, (addr, adind) => {
        if (angular.isDefined(addr.is_default) && addr.is_default) {
          if (angular.isDefined(addr.company_name) && addr.company_name !== "" && addr.company_name !== null) {
            $scope.deliveryForm.billing_company_name = angular.copy(addr.company_name);
          }
          if (angular.isDefined(addr.gstin) && addr.gstin !== "" && addr.gstin !== null) {
            $scope.deliveryForm.billing_gstin = angular.copy(addr.gstin);
          }
          if (angular.isDefined(addr.address_line) && addr.address_line !== "") {
            $scope.deliveryForm.billing_address_line = angular.copy(addr.address_line);
          }
          if (angular.isDefined(addr.area) && addr.area !== "") {
            $scope.deliveryForm.billing_area = angular.copy(addr.area);
          }
          if (angular.isDefined(addr.city) && addr.city !== "") {
            $scope.deliveryForm.billing_city = angular.copy(addr.city);
          }
          if (angular.isDefined(addr.state) && addr.state !== "") {
            $scope.deliveryForm.billing_state = angular.copy(addr.state);
          }
          if (angular.isDefined(addr.pincode) && addr.pincode !== "") {
            $scope.deliveryForm.billing_pincode = angular.copy(addr.pincode);
          }
          if (angular.isDefined(addr.landmark) && addr.landmark !== "") {
            $scope.deliveryForm.billing_landmark = angular.copy(addr.landmark);
          }
          if (angular.isDefined(addr.contact_no) && addr.contact_no !== "") {
            $scope.deliveryForm.billing_contact_no = angular.copy(addr.contact_no);
          }
        }
      });
        ...
        if (result !== null && angular.isDefined(result.data) && result.data !== null && result.data.length > 0) {
          angular.forEach(result.data, (ord) => {
            if (angular.isDefined(ord) && angular.isDefined(ord.inwards) && ord.inwards !== null && ord.inwards.length > 0) {
              ...
              $scope.deliveryData.orders.push(angular.copy(ord));
            }
          });
          setTimeout(() => {
  ...
  $scope.saveDelivery = function (opt) {
    ...
    validateField.validate($scope.deliveryForm, orderfield, ordermsgData).then((orderMsg) => {
      if (angular.isDefined(orderMsg) && orderMsg !== null && orderMsg !== "") {
        ...
        validateField.validateGroup($scope.deliveryForm.outwardData, outwardfield, outwardmsgData).then((outwardMsg) => {
          if (angular.isDefined(outwardMsg) && outwardMsg !== null && outwardMsg !== "") {
            Notification.error(outwardMsg);
          } else {
            validateOutwarddata($scope.deliveryForm.outwardData).then((valid) => {
              if (angular.isDefined(valid) && valid) {
                $rootScope.orderpageLoader = true;
                const obj = {};
                obj.deliveryForm = angular.copy($scope.deliveryForm);

                DeliveryService.save(obj, (result) => {
  ...
  function getreceived (inwards){
    ...
    if (inwardCount === $scope.deliveryForm.inwards.length) {
      deferred.resolve($scope.deliveryForm.inwards);
    }
  ...
  $scope.selectOrder = function (order) {
    ...
    $scope.deliveryForm.outwardData = [];
    ...
    if (angular.isDefined(order) && order !== null && angular.isDefined(order._id) && angular.isDefined(order.inwards) &&
      ...
      $scope.deliveryForm.order_id = angular.copy(order._id);
  ...
  $scope.selectInward = function (inward) {
    ...
    $scope.deliveryForm.inwards.indexOf(inward) > -1) {
      if ($scope.deliveryForm.outwardData.length > 0) {
        let indexexist = -1;
        angular.forEach($scope.deliveryForm.outwardData, (inw, ind) => {
          if (angular.isDefined(inw.inward_data_id) && inw.inward_data_id === inward.inward_data_id) {
            indexexist = ind;
            inward.is_checked = false;
          }
          if (ind === $scope.deliveryForm.outwardData.length - 1) {
            if (indexexist > -1) {
              $scope.deliveryForm.outwardData.splice(indexexist, 1);
            } else {
              inward.is_checked = true;
              $scope.deliveryForm.outwardData.push(angular.copy(inward));
            }
          }
        });
      } else {
