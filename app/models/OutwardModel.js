...
function isUnique(value, res) {
  if (value) {
    mongoose.models.delivery.count({_id: {$ne: this._id}, outward_no: this.outward_no, division_id: this.division_id}, (err, count) => {
      if (err) {
        return res(err);
      }
      res(!count);
...
const Schema = new mongoose.Schema({
  outward_no: {type: String,
    trim: true,
    required: true,
    validate: [
      {isAsync: true, validator: isUnique, msg: "Outward no already exists"},
    ]},
  outward_serial_no: {type: Number, required: true},
  outward_date: {type: Date, default: Date.now},
  ...
  inward: [{type: mongoose.Schema.Types.ObjectId, ref: "contractinward"}],
  ...
  outward_status: {type: String, enum: ["In Progress", "Completed"], default: "In Progress"},
});
...
module.exports = mongoose.model("outward", Schema);
